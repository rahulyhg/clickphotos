...
angular.module('phonecatControllers', ['templateservicemod', 'navigationservice', "jsonservicemod", 'ui.bootstrap', 'ui.select', 'ngAnimate', 'toastr', 'ngSanitize', 'angular-flexslider', 'ui.tinymce', 'imageupload', 'ngMap', 'toggle-switch', 'cfp.hotkeys', 'ui.sortable'])
    ...
    .controller('DetailFieldCtrl', function ($scope, TemplateService, NavigationService, JsonService, $timeout, $state, $stateParams, $uibModal, toastr) {
        ...
        $scope.refreshTags = function (search) {
            if ($scope.type.url !== "" && $scope.type.params) {
                //console.log("m in else if", $stateParams.keyword);
                NavigationService.apiCall($scope.type.url, $stateParams.keyword, function (data1) {
    ...
    .controller('PhotoContestCtrl', function ($scope, TemplateService, NavigationService, JsonService, $timeout, $state, $stateParams, $uibModal, toastr) {
        ...
        // $scope.conPhotoData = [];
        // //  END FOR EDIT
        // $scope.editBoxCustomPhotoContest = function (project) {
        //     var abc = {};
        //     abc._id = project;
        //     //abc.testid = value;
        //     //console.log("PROJECT IMAGE _____-------", abc);
        //     NavigationService.apiCall("photoContest/findPhotographer", abc, function (data) {
        ...
        $scope.removePhotos = function (index) {
            _.remove($scope.contestData.Photos, function (n) {
                return n == $scope.contestData.Photos[index];
            });
            console.log("child------------", $scope.contestData);
